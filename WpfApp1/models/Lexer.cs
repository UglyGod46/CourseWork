        ...
        while (_position < _input.Length)
        {
            ...
            if (char.IsLetter(currentChar))
            {
                ...
                if (token.Type == TokenType.Keyword && token.Value == "function")
                {
                    ...
                    // Проверяем, что следующий символ — это открывающая скобка '('
                    if (_position >= _input.Length || _input[_position] != '(')
                    {
                        tokens.Add(new Token
                        {
                            Type = TokenType.Error,
                            Value = "Ошибка: После названия функции должна быть открывающая скобка '('",
                            StartIndex = _position,
                            EndIndex = _position
                        });
                        return tokens; // Завершаем анализ
                    ...
                    // Добавляем открывающую скобку в список токенов
                    tokens.Add(new Token
                    {
                    ...
                    {
                        // Проверяем, что текущий символ — это буква (начало идентификатора)
                        if (!char.IsLetter(_input[_position]))
                        {
                            tokens.Add(new Token
                            {
                                Type = TokenType.Error,
                                Value = "Ошибка: Ожидается идентификатор параметра",
                                StartIndex = _position,
                                EndIndex = _position
                            });
                            return tokens; // Завершаем анализ
                        ...
                        Token paramToken = ParseIdentifierOrKeyword();
                        tokens.Add(paramToken);

                        // Пропускаем пробелы после идентификатора
                        MatchSeparator();

                        // Проверяем, что следующий символ — это запятая или закрывающая скобка
                        if (_position < _input.Length && _input[_position] == ',')
                        {
                            // Добавляем запятую в список токенов
                            tokens.Add(new Token
                            {
                                Type = TokenType.Punctuation,
                                Value = ",",
                            ...
                            // Проверяем, что после запятой идет идентификатор
                            if (_position >= _input.Length || !char.IsLetter(_input[_position]))
                            {
                                tokens.Add(new Token
                                {
                                    Type = TokenType.Error,
                                    Value = "Ошибка: После запятой должен быть идентификатор параметра",
                                    StartIndex = _position,
                                    EndIndex = _position
                                });
                                return tokens; // Завершаем анализ
                        ...
                        else if (_position < _input.Length && _input[_position] != ')')
                        {
                            ...
                            {
                                Type = TokenType.Error,
                                Value = "Ошибка: Ожидается запятая или закрывающая скобка",
                                StartIndex = _position,
                                EndIndex = _position
                            });
                            return tokens; // Завершаем анализ
                    ...
                    // Проверяем, что следующий символ — это закрывающая скобка ')'
                    if (_position >= _input.Length || _input[_position] != ')')
                    {
                        tokens.Add(new Token
                        {
                            Type = TokenType.Error,
                            Value = "Ошибка: Ожидается закрывающая скобка ')'",
                            StartIndex = _position,
                            EndIndex = _position
                        });
                        return tokens; // Завершаем анализ
                    ...
                    // Добавляем закрывающую скобку в список токенов
                    tokens.Add(new Token
                    {
                        Type = TokenType.Punctuation,
                        Value = ")",
